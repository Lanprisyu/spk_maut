						...
						<tr align="center">
							...
							<?php foreach ($kriteria as $key) : ?>
								<td>
									...
									$data_pencocokan = $this->Perhitungan_model->data_nilai($keys->id_alternatif, $key->id_kriteria);
									$min_max = $this->Perhitungan_model->get_max_min($key->id_kriteria);
									$hasil = @(($data_pencocokan['nilai'] - $min_max['min']) / ($min_max['max'] - $min_max['min']));
									echo $hasil;
									?>
								</td>
							<?php endforeach ?>
...
<div class="card shadow mb-4">
	<!-- /.card-header -->
	<div class="card-header py-3">
		<h6 class="m-0 font-weight-bold text-dark"><i class="fa fa-table"></i> Perkalian Matrik Normalisasi Dengan Bobot Kriteria</h6>
	</div>
						...
						<tr align="center">
							...
							<td>SUM
								...
								foreach ($kriteria as $key) :
									$data_pencocokan = $this->Perhitungan_model->data_nilai($keys->id_alternatif, $key->id_kriteria);
									$min_max = $this->Perhitungan_model->get_max_min($key->id_kriteria);
									$hasil_normalisasi = @(($data_pencocokan['nilai'] - $min_max['min']) / ($min_max['max'] - $min_max['min']));
									$bobot = $key->bobot;
